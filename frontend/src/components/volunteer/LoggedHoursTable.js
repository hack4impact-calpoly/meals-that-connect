...
const EditableCell = (cellProperties, width) => {
    ...
    let data = cellProperties['row']['original']
    ...
    const updateDatabase = async (data, key, value) => {
      const updateData = {
        logID: data._id,
        key: key,
        value: value
    ...
    if (column == 'Date') {
      return (
          <span><input style={{width: width - 1.25, textAlign: 'center'}} type="date" defaultValue={value.slice(0,10)} onChange={e => handleChange(e.target.value)} onBlur={e => updateDatabase(data, key, e.target.value)}/></span>
      )
    }
    else {
      return (
        <span><input style={{width: width - 1.25, textAlign: 'center'}} value={value} onChange={e => handleChange(e.target.value)} onBlur={e => updateDatabase(data, key, e.target.value)}/></span>
    )
    }
}
